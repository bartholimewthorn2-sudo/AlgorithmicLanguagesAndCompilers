...
/*
So I've written the majority of the logic of the code and operations
I've already added all the necessary function calls, what needs done:
I labled everywhere I think output was needed with outputFile <<
These need filled in, I am having trouble understanding label and 
how its values are generated, if you know and can fix the commented
code in the fucntions that still need the outputFile fixed great,
if not let me know and I'll probably need to set a function to generate
label names much like temps. If I missed something let me know.
*/
using namespace std; 
...
enum opers {PROGRAM, END, READ, WRITE, PLUS, MINU, NEG, NOT, MULTI, DIV, MOD, AND, OR, EQUA, ASSI};
...
void EmitNegativeCode(string, string);
...
array<string, 5> errors = {"lexical", "semantic", "syntactic", "Stage 1", "Stage 2"}; //For errors processing
...
vector<string> keyIds = {"program", "begin", "end", "var", "const", "integer", 
		...
		"if", "then", "else", "repeat", "while", "do", "until"}; //List of all intital keywords
bool recur = false;
int lineNum = 1;
...
void BeginEndStmt() //token should be "begin"
{
	...
	else if (token == ".")
	{
		...
		if (token != END_OF_FILE)
			errorHandle(errors[2], 18);
	}
	else 
	{
		NextToken();
		if (token != "begin")
			errorHandle(errors[2], 20);
	}
		...
		else //create table entry
		{
			...
			else
			{
				...
				else if (inType == prog)
				{
					...
					entry alpha;
					alpha.externalName = externalName;
					datFileNumber = alpha.externalName;
					alpha.internalName = interName;
					alpha.dataType = inType;
					alpha.mode = inMode;
					alpha.value = inValue;
...
void errorHandle (string errTyp, int errNum) //Need to print out the line number the of the error and determine the errors type
{
	...
	*/
	...
	else if (errTyp == "syntactic")
	{
		...
		{
			listingFile << "'.' or ';' expected after end statement\n";
			errCnt++;
		}
		else if (errNum == 14)
		{
		...
		{
			listingFile << "Unexpected end of file, Expected either 'begin' or '.' after the final end\n";
			errCnt++;
		}
		else 
		{
	...
	else if (errTyp == "Stage 2")
	{
		listingFile << "Compiler Error: ";
		if (errNum == 1)
		{
			listingFile << "Expected 'then' after if statement\n"
			errCnt++;
		}
		else if (errNum == 2)
		{
			listingFile << "Expected 'do' after while statement\n";
			errCnt++;
		}
		else if (errNum == 3)
		{
			listingFile << "Expected 'until' after repeat statement\n";
			errCnt++;
		}
		else
		{
...
//if name is a literal, also create a symbol table entry for it
{
	...
	{
		if (name == symbolTable[i].internalName) //This may not work Not sure if its a good literal test or not
		{
...
{
	enum opers operat;
	if (operators == "program")
	{
		operat = PROGRAM;
	}
	else if (operators == "end")
	{
		operat = END;
	}
	else if (operators == "read")
	{
		operat = READ;
	}
	else if (operators == "write")
	{
		operat = WRITE;
	}
	else if (operators == "+")
	...
	switch (operat)
	{
		case PROGRAM: objectFile << "STRT  " << "NOP          " << datFileNumber << " - " << names << "\n";
		case END : objectFile << "     HLT" << "\n";
			REGQ = ""; //REGQ stack needs to be popped releasing all data 
			PrintSymbolTable();
			objectFile << "      END" << " STRT" << "     End; begin execution at label STRT" << "\n";
		...
		case PLUS  : EmitAdditionCode(operand1, operand2); //this must be binary "+"
		case MINU  : EmitSubtractionCode(operand1, operand2); //this must be binary "-"
		case NEG   : EmitNegativeCode(operand1, operand2); //this must be unary "-"
		case NOT   : EmitNotCode (operand1, operand2);
		case MULTI : EmitMultiplicationCode(operand1, operand2);
		...
		case EQUA  : EmitEqualsCode(operand1, operand2);
		case ASSI  : EmitAssignCode(operand1, operand2);
...
void EmitModuloCode(string operand1, string operand2)
{
	...
	for (int i = 0; i < symbolTable.size(); i++)
	{
		if (REGA == symbolTable[i].value)
		{
			if (REGA != operand1 && REGA != operand2)
			{
				...
				REGA = ""; //Clear REGA 
				objectFile << "n spaces" << "STA" << "2000" << endl; //Store temp into memory as T#
	...
	if (REGA != operand2)
	{
		REGA = "";
		REGA = operand2;
		objectFile << "n spaces" << "LDA" << "1200" << endl; //Load operand2 into REGA
	}
	objectFile << "n spaces" << "IDV" << "2500" << endl; //Emit Addition Code
...
{
	if (operand1 == "true")
		objectFile << "n spaces" << "LDA" << "1200" << endl; //Load false into AREG I think 
	else if (operand1 == "false")
		objectFile << "n spaces" << "LDA" << "1201" << endl; //Load false into AREG I think 
	else
		errorHandle(errors[3], 3);
...
void EmitAdditionCode(string operand1,string operand2) //add operand1 to operand2
{
	...
	for (int i = 0; i < symbolTable.size(); i++)
	{
		if (REGA == symbolTable[i].value)
		{
			if (REGA != operand1 && REGA != operand2)
			{
				...
				symbolTable.back().value = REGA; //Deassign REGA by adding to temp
				REGA = ""; //Clear REGA 
				objectFile << "n spaces" << "STA" << "2000"; //Store temp into memory as T#
				for (int j = 0; j < symbolTable.size(); j++)
				{
					if (symbolTable[j].value == symbolTable.back().value)
					{
	...
	objectFile << "n spaces" << "IAD" << "1400" << endl; //Emit Addition Code
	
	
	for (int i = symbolTable.size(); i > 0 ; i--)
	{
		char help = symbolTable[i].internalName.front();
		if (help == 'T')
		{
			symbolTable[i].alloc = NO;
			FreeTemp();
		}
	}
	GetTemp();
...
void EmitMultiplicationCode(string operand1,string operand2) //multiply operand2 by operand1
{
	...
	objectFile << "n spaces" << "IMU" << "2400" << endl; //Emit Addition Code
...
void EmitOrCode(string operand1,string operand2) //"or" operand1 to operand2
{
	...
	objectFile << "n spaces" << "AZJ" << "3600" << endl;
...
void EmitEqualsCode(string operand1,string operand2) //test whether operand2 equals operand1
{
	...
	objectFile << "n spaces" << "LDA" << "1200" << endl;
	...
	objectFile << "n spaces" << "UNJ" << "7500" << endl;
...
void EmitNegativeCode(string operand1, string operand2)
{
	string tstTyp1 = typeid(operand1).name();
	string tstTyp2 = typeid(operand2).name();
...
storeType WhichType(string name)
{
	...
	else 
	{
		for (int i = 0; i < symbolTable.size(); i++)
		{
			...
			{
				storeType help = symbolTable[i].dataType;
				return help;
			}
		}
	}
...
{
	string tempLabel;
	//Not sure how label works, If you can make sure it points to the NEXT label that would be helpful
	tempLabel = label; //assign next label to tempLabel;
	if (operand == "true" || operand == "false")
	{
		objectFile << // Condition Boolean emit instruction to set the condition code depending on the value of operand;
		if (operand)
		{
			objectFile << //Emit code for jump to tempLabel
			for (int i = 0; i < symbolTable.size(); i++)
			{
				if (symbolTable[i].value == operand)
				{
					symbolTable[i].internalName == " ";
					symbolTable[i].value == " ";
					symbolTable[i].alloc == NO;
					FreeTemp();
				}
			}
			objectFile << //Clear the registers, both I think'
			return;	
		}
	}
	else 
	{
		objectFile << //Condition Interger emit instruction to set the condition code depending on the value of operand;
		if (/*I'm not positive what to do here, I don't know what I need to test operand against to confirm its now 0*/)
		{
			objectFile << //Emit code for jump to tempLabel
			for (int i = 0; i < symbolTable.size(); i++)
			{
...
void EmitElseCode(string operand) //emit code that follows else clause of if statement
{
	string tempLabel;
	tempLabel = label; //assign next label to tempLabel;
	objectFile << UNCONDITIONAL JUMP << TEMPLABEL1 ;//emit instruction to branch unconditionally to tempLabel;
	objectFile << ;//emit instruction to label this point of object code with the argument operand;
	PushOperand(tempLabel);
	objectCode <<; //deassign operands from all registers
}
void EmitPostIfCode(string operand) //emit code that follows end of if statement
{
	objectFile << operand; //CHECK THIS //emit instruction to label this point of object code with the argument operand;
	objectFile << ;//deassign operands from all registers
}
void EmitWhileCode() //emit code that follows while
{
	string tempLabel;
	tempLabel = label;
	objectFile << //emit instruction to label this point of object code as tempLabel;
	PushOperand(tempLabel);
	objectFile << //deassign operands from all registers
}
...
{
	objectCode << UNCONDITIONAL JUMP << OPERAND 2 //emit instruction which branches unconditionally to the beginning of the loop, i.e., to the
	//value of operand2;
	objectFile << //emit instruction which labels this point of the object code with the argument operand1;
	objectFile << //Clear the registers, both I think'
}
void EmitRepeatCode() //emit code that follows repeat
{
	//there is a "var" before the string in pseudo not sure why keep it noted
	string tempLabel;
	tempLabel = label;
	objectFile << //emit instruction to label this point in the object code with the value of tempLabel;
	PushOperand(tempLabel);
	objectFile << //deassign operands from all registers
}
 ...
 //predicate. operand2 is the label that points to the beginning of the loop
{
	if (operand1 == "true" || operand1 == "false")
	{
		...
		if (operand1)
		{
			objectFile << //Emit code for jump to operand2
			for (int i = 0; i < symbolTable.size(); i++)
			{
				if (symbolTable[i].value == operand)
				{
	...
	for (int i = 0; i < symbolTable.size(); i++)
	{
		if (symbolTable[i].value == operand1)
		{
			symbolTable[i].internalName == " ";
			symbolTable[i].value == " ";
			symbolTable[i].alloc == NO;
...
void execStmt()
{
	...
	if (nonKeyId(token))
	{
		...
		if (token == ":=")
		{
			...
			{
				NextToken();
				string call1 = PopOperator();
				string call2 = PopOperand();
				string call3 = PopOperand();
				Code(call1, call2, call3);
			}
		}
	}
	//ReadStmts
