...
//if name is a literal, also create a symbol table entry for it
{
	...
	{
		storeType type = //Write and call whichType function
...
{
	if (operatorStk.top() != NULL)
	{
		rtrnVal = operatorStk.top();
		operatorStk.pop();
		return rtrnVal;
	}
	else 
...
{
	if (!is_integer(operand1) || !is_integer(operand2))
		errorHandle(errors[3], 3);
	if (REGA == temp)
	{
		if (REGA != operand1 && REGA != operand2)
		{
			REGA = ""; //May need to be calling Code here to clear REGA 
			Code(=, temp) //Store temp into REGQ
			for (int i = 0; i < symbolTable.size(); i++)
			{
				if (symbolTable[i].value = temp)
				{
	...
	else if (REGA != operand1 && REGA != operand2)
	{
		REGA = "";
		Code(=, operand2)//Move Operand2 into REGA
	}
	Code(+, operand1)//
...
{
	if (operand1.storType != operand2.storType)
		errorHandle(errors[3], 4);
	
	if (operand1.mode != VARIABLE)
